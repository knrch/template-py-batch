...
#
# Wraps `uv` so that any change to uv.lock is followed by `osv-scanner` +
# `pip-audit` against the new lockfile. If ANY new advisory (any severity)
# is introduced that was not present before, the lockfile is restored and
...
#
# Exit codes:
#   0 - clean (no NEW vulnerabilities of any severity introduced)
#   1 - rejected: new advisory introduced; lockfile restored
#   2 - tool error
# 127 - missing required tool
...
if [[ -f "$LOCKFILE" ]]; then
  ...
  ( cd "$REPO_ROOT" && uv export --no-hashes --no-dev --format requirements-txt >"$PRE_REQ" 2>/dev/null ) || true
  if [[ -s "$PRE_REQ" ]]; then
...
( cd "$REPO_ROOT" && uv export --no-hashes --no-dev --format requirements-txt >"$POST_REQ" 2>/dev/null ) || true
if [[ -s "$POST_REQ" ]]; then
