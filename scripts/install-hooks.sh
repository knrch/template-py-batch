...
#      from lefthook.yml. Lets contributors who already use lefthook keep their config.
...
MISSING=0
for tool in osv-scanner pip-audit uv trivy gitleaks semgrep; do
...
if [[ "$MISSING" -eq 1 ]]; then
  ...
  echo "    brew install uv  # if not already installed" >&2
  echo
