#!/usr/bin/env bash
# One-shot security audit: uv.lock (osv-scanner + pip-audit), optional Trivy on
# pinned Docker bases, gitleaks (per-path source scan), and Semgrep on src.
#
# Usage:
...
Usage: $(basename "$0") [options]
  ...
  --supply-chain-only           uv.lock + pip-audit + optional trivy only.

With no flags: lockfile audits + gitleaks + semgrep are run.
USAGE
      exit 0 ;;
    *)
...
echo
log_info "=== Python lockfile (uv.lock) — all severities ==="
osv-scanner --lockfile "$REPO_ROOT/uv.lock" || FAIL=1
TMPREQ="$(mktemp)"
trap 'rm -f "$TMPREQ"' EXIT
( cd "$REPO_ROOT" && uv export --no-hashes --no-dev --format requirements-txt >"$TMPREQ" )
...
# Digest-pinned bases — MUST match Dockerfile and docker-compose.yml.
# Replace PIN_ME_AT_BOOTSTRAP with the digest you committed in those files.
RUNTIME_IMAGES=(
