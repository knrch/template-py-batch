...
- **Container bases**: digest-pinned mainstream public images
  (`python:3.14-slim@sha256:...`, `postgres:16-alpine@sha256:...`).
  Renovate keeps digests fresh. cosign / Sigstore is not used.
- **Dependencies**: `uv.lock` is the source of truth. Every install
  goes through `scripts/safe-uv.sh`, which rejects any new advisory
  (any severity) introduced by an operation.
- **Source secrets**: blocked at commit time by gitleaks; allowlist
  in `.gitleaks.toml`.
- **SAST**: semgrep `p/python` on staged files at pre-commit; full
  tree on scheduled CI.
...
| semgrep `p/python` | Python SAST patterns | pre-commit (staged) + CI (full tree) |
| osv-scanner | Cross-ecosystem CVE backstop on `uv.lock` | pre-commit on dep changes + scheduled CI |
...
| safe-uv.sh | Gates every dependency add/remove/sync | local, on demand |
...
- Renovate proposes new digests weekly (Monday before 6am
  Asia/Tokyo).
- After merging a digest bump, mirror the digest into
  `scripts/audit-all.sh` (the `RUNTIME_IMAGES` array). The three
  locations — Dockerfile, docker-compose.yml, audit-all.sh — must
  agree.
